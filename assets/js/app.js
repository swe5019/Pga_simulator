...
/**
 * Trim the built pool down to the top N lineups by the active "Optimize for" metric,
 * re-enforcing per-golfer exposure caps on the survivors. This is the funnel step:
 * build a wide pool, then keep only the number you'll actually enter, without blowing
 * past your exposures. The full pool is regenerated by rebuilding on the Build tab.
 */
function trimPool() {
  if (!State.build || !State.build.lineups.length) return;
  const n = parseInt($('#trimN').value, 10);
  if (!n || n < 1) { $('#buildStatus').textContent = 'Enter how many lineups to trim to.'; return; }
